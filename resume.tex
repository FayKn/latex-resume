\documentclass{article}
% Packages for formatting
\usepackage[a4paper,margin=1in]{geometry}
\usepackage{enumitem}
\usepackage{parskip}
\usepackage{hyperref}
\usepackage{fontspec}
\usepackage{titlesec}

\titlespacing*{\section}
{0pt}{10pt}{5pt}

\setmainfont[
    BoldFont={montserrat-variablefont_wght}, % Specify the bold font variant
    BoldFeatures={Weight=700}, % Set the desired weight for bold (e.g., 700)
    Weight=400 % Set the desired weight for normal text (e.g., 500)
]{montserrat-variablefont_wght} % Replace with your desired system font

\begin{document}
\pagenumbering{gobble}

% Your name and contact information
\begin{center}
    \textbf{\Huge Fay Knol}

    \vspace{5pt}
    
    Almere, The Netherlands
    
    \vspace{5pt}
    
    Phone: (+31) 619541435 \quad Email: fay@fayk.nl \quad LinkedIn: \href{https://www.linkedin.com/in/faykn}{linkedin.com/in/faykn}
\end{center}

\noindent\makebox[\linewidth]{\rule{\paperwidth}{0.4pt}}

% Education section
\section*{Education}
\textbf{Hogeschool Utrecht} \hfill 2023-2028 \\
Bachelor, HBO-ICT (Open-ICT) \hfill

\textbf{ROC Flevoland} \hfill 2020-2022 \\
MBO 4, Software Developer \hfill

\noindent\rule{\linewidth}{0.4pt}

% Work experience section
\section*{Work Experience}
\textbf{Yivi} \hfill Sep 2025-Jan 2026 \\
\textit{Intern}, Utrecht \hfill
\begin{itemize}[noitemsep]
    \item My assignment this internship is to build MFA funtionality into the Yivi app to make it the everything login app.
\end{itemize}
\textbf{Hogeschool Utrecht} \hfill Feb 2025-Jun 2025 \\
\textit{Student Assisant}, Utrecht \hfill
\begin{itemize}[noitemsep]
    \item I helped first year students with their programming assignments, questions about the Open-ICT system and give feedback on work.
     I've learnt a lot about teaching and how to give better feedback to students.
\end{itemize}
\textbf{Jaspers Media} \hfill May 2022-Aug 2024 \\
\textit{Full stack developer}, Amsterdam \hfill
\begin{itemize}[noitemsep]
    \item After my internship I was hired on as a developer here, during my gap year (-1 month).
        I was responsible for all critical bugfixes and pull requests as well as developing new features and making large preformance improvements, often being over 10\%.
\end{itemize}
\textbf{Jaspers Media} \hfill Nov 2021-May 2022 \\
\textit{Intern Software developer}, Amsterdam \hfill
\begin{itemize}[noitemsep]
    \item At the start of my internship I was mostly busy rewriting our websites \href{https://luier.nl}{luier.nl} and \href{https://flap.nl}{flap.nl} (with a complete redesign) from Innertia.js to a Nuxt 3 and Laravel app with a focus on SEO and preformance.
    \item Near the end of the internship the rewrite successfully went live.
    \item I frequently collaborated with other interns and coworkers new on the project and taught them how we work with Laravel and Nuxt being the one people went to when they couldn't figure something out.
\end{itemize}

\noindent\rule{\linewidth}{0.4pt}

\section*{Projects}
\textbf{Game Panel Addons} \hfill Okt 2024-Present \\
\begin{itemize}[noitemsep]
    \item Together with my friend we sell functional additions for the popular open source game panel Pterodactyl, these have already sold over 200 copies after 6 months being used by over 12,000 end-users.
    \item I was responsible for the back-end made using Go and Laravel.
    \item During this project I continuously learn more about customer relation management, service and shipping reliable code. 
\end{itemize}

\newpage


\noindent\rule{\linewidth}{0.4pt}

% Skills section
\section*{Hard Skills}
These skills are roughy ordered by how comfortable I am with them, with the first being the most comfortable or experienced.
\begin{itemize}[noitemsep]
    \item \textbf{Languages:} Dutch (native), English (fluent)
    \item \textbf{Programming Languages:} Go, PHP, JavaScript
    \item \textbf{Frameworks:} Laravel, Chi (Go), Nuxt, TailwindCSS, Echo (Go)
    \item \textbf{Tools:} Git, Docker, Asana, Figma, Azure DevOps, Jira
    \item \textbf{Databases:} MySQL/MariaDB, Redis, MongoDB
    \item \textbf{Other:} Linux, Nix
\end{itemize}

\end{document}